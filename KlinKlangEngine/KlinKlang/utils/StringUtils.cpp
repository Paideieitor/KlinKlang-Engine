...
std::wstring Utf8ToWide(const string& str)
{
    std::wstring output;
    output.resize(utf8towchar(str.c_str(), SIZE_MAX, nullptr, 0) - 1);
    utf8towchar(str.c_str(), SIZE_MAX, &output[0], output.length());
    return output;
}

std::string WideToUtf8(const wstring& wstr)
{
    std::string output;
    output.resize(wchartoutf8(wstr.c_str(), SIZE_MAX, nullptr, 0) - 1);
    wchartoutf8(wstr.c_str(), SIZE_MAX, &output[0], output.length());
    return output;
}
...
vector<string> StringToStringVector(const string& input, char separator)
{
    ...
    for (u32 idx = 0; idx < (u32)input.size(); ++idx)
    {
        ...
        {
            output.emplace_back(input.substr(start, idx - start));
            start = idx + 1;
        }
    }
    return output;
...
vector<u32> StringToU32Vector(const string& input, char separator)
{
    ...
    for (u32 idx = 0; idx < (u32)input.size(); ++idx)
    {
        if (input[idx] == separator)
        {
            output.emplace_back((u32)stoi(input.substr(start, idx - start)));
            start = idx + 1;
        }
    }
    return output;
